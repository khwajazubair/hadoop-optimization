...
def hadoop_run_workload(pm, exp_number, workload):
    ...
    scp_file_from_host("logs_summary.out", "runs_2/logs_summary.out%s.%s" % (exp_number, time_before), IP)
    #scp_file_from_host("hadoop-3.0.0-SNAMPSHOT/logs/facebook_log.tsv", "runs_2/facebook_log.tsv.%s.%s" % (exp_number, time_before), get_ip_for_instance(HADOOP_NN))

    #scp_file_from_host("hadoop-3.0.0-SNAPSHOT/logs/*history*.log", "runs_2/history.%s.%s" % (exp_number, time_before), IP)
    scp_folder_from_host("hadoop-3.0.0-SNAPSHOT/logs/fairscheduler/*", "runs_2/",IP)
    execute_on_vm(get_ip_for_instance(HADOOP_NN), "rm -r hadoop-3.0.0-SNAPSHOT/logs/fairscheduler")
    scp_folder_from_host("hadoop-3.0.0-SNAPSHOT/logs/*", "runs_2/",IP)
    scp_folder_from_host("logs/*", "runs_2/",IP)

    # Pull in all the NodeManager logs (should be done away with as soon as
    # we can get log aggregation to work)
    for i in range(2, num_hadoop + 1):
        scp_folder_from_host("~/hadoop-3.0.0-SNAPSHOT/logs/userlogs", "runs_2/userlog-%s" % i, get_ip_for_instance(HADOOP_HOSTNAME_PREFIX + "-%s" % i))
